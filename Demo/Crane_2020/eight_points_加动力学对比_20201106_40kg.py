...
adjust3_rom = -1.8  # 1.5
adjust3_fem = 0.5  # 0.5
""" 40 700 : 2号点 gs -x rom fem +x"""
adjust_gs = -3  # 2.1
adjust2_rom = 15.5  # 2.5
adjust2_fem = 15.5  # 2.5
""" 40 700 : 4号点"""
# adjust_gs = 5  # 5
adjust4_rom = 12.5  # 12.5
adjust4_fem = 16  # 16
# 输出哪个点
which_point = 2
...
simulation_number = 700  # 300 400 200 600 500 700 800 仿真的重物重力(N)
row_index = 3  # 因为应变采集仪在开始测量的时候总是慢一拍，所以要调整到和仿真的一致，从3开始调
simulation_index = 0  # 仿真的偏移值
stress_y_limit = 55  # y刻度最大值
stress_y_limit_separate = 5  # y刻度分度数
x_tick = 140.1  # x的最大值 140.1
...
for i in [which_point_dic[which_point]]:
    ...
    rom_fem = r2(np.asarray(arr_4), np.asarray(arr_3))
    print("gs", gs_real, "\r\nrom", rom_real, "\r\nfem", fem_real)
    print("rom_fem", rom_fem)
