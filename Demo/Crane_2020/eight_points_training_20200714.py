...
y_real_ = list_all_points_diverse_condition[4][2]  # 第一层的力大小的顺序 300 400 200 600 500 700 800 仿真的重物重力(N)
# print(len(list_all_points_diverse_condition[0][0]))
# 【训练加预测】
# 【训练】取4个样本点0 25 50 73
x_train_ = np.array([0, 24, 48, 72]).reshape(-1, 1)
# 第一层的力大小的顺序 300 400 200 600 500 700 800 仿真的重物重力(N)
y_train_ = list_all_points_diverse_condition[3]  # 这里需要更改，不同的力使用对应的数组
list_GPR = []
for i in range(8):
...
# 测试数据文件名，实时仿真数据的文件名
load = "40"  # 10 15 20 23 23_2 25_1 25 30 35 40 实际的重物质量(kg)
simulation_number = 600  # 300 400 200 600 500 700 800 仿真的重物重力(N)
row_index = 3  # 因为应变采集仪在开始测量的时候总是慢一拍，所以要调整到和仿真的一致，从3开始调
