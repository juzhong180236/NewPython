...
def read_excel(path=None, sheet_number=0, sheet_name=None, row_index=1, column_index=1, row_num=None, column_num=None,
               y_column=1):
    """
    ...
    if row_num is not None and column_num is not None:
        ...
        row_num_read = sheet.max_row - row_index + 1
        column_num_read = sheet.max_column - column_index + 1
    data = np.empty((row_num_read, column_num_read))  # 用于存储数据的array，相较于np.ones只分配不初始化
    for i in range(row_index, row_index + row_num_read):
        for j in range(column_index, column_index + column_num_read):
            data[i - row_index][j - column_index] = float(sheet.cell(row=i, column=j).value)
    return data
