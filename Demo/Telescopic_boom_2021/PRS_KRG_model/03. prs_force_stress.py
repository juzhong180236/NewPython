...
预测的输入
"""
x_train = pd.read_csv(path_read + r'Ridge\\' + 'delta_F.csv').values
print("The model is building...")

stresses_prediction_list = []
for _i_list in range(4):
    stress_prediction = pd.read_csv(path_read + r'Ridge\\' + 'delta_S' + str(_i_list + 1) + '.csv')
    stress_prediction_index = np.hstack((np.array([cd_index_list[_i_list]]).reshape(-1, 1),
                                         stress_prediction.values))
    stresses_prediction_temp_list = []
    for _i, _ in enumerate(stress_prediction_index):
        if stress_prediction_index[_i][0] in set_ele_surface_list[_i_list]:
            stresses_prediction_temp_list.append(stress_prediction_index[_i][1:])
...
for i_component, _ in enumerate(stresses_prediction_list):
    ...
    for i_node, _ in enumerate(np_array_component_stresses):
        prs_stress_node = PRS(m=1)
        if i_node == 0:
            m = prs_stress_node.m
            gram_matrix = prs_stress_node.calc_gram_matrix(x_train)
...
"""
dict_prs_model = {
    "delta_stress_w": list_w_stress,
}
json_prs_model = json.dumps(dict_prs_model)
with open(path_prefix + r'pre_telescopic_boom_v3.0\\' + r"prs\\" + prs_type + r"\\"
          + path_switch[4:-7] + "_delta_s_prs.json", "w") as f:
