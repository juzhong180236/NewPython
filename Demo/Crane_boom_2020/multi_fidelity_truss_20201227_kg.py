...
def create_figure(_ax, _test_X, _test_Y, _predict_stress_results, _train_X, _train_Y, _real_stress,
                  color_map='rainbow', point_color='k', _alpha=0.6, _count=50):
    _ax.plot_surface(
        _test_X,
        _test_Y,
...
def create_figure_co(_ax, _test_X, _test_Y, _predict_stress_results, color_map='rainbow', _alpha=0.6, _count=50):
    _ax.plot_surface(
        _test_X,
        _test_Y,
        _predict_stress_results,
...
for i_point in [0]:
    ...
    create_figure_co(ax, verification_X, verification_Y,
                     list_test_predict_stress_multi[i_point], 'viridis',
                     _alpha=__alpha,
                     _count=__count,
                     )
    create_figure_co(ax, verification_X, verification_Y,
                     array_real_stress_verification[i_point].T, 'inferno',
                     _alpha=__alpha,
                     _count=__count,
                     )
    ax.set_xlabel("The mass of lifting load (Kg)", fontsize=12)
    ax.set_ylabel("The degree of luffing angle (deg)", fontsize=12)
    ax.set_zlabel('Stress (Mpa)', fontsize=12)
    BIGGER_SIZE = 12
    plt.tick_params(labelsize=BIGGER_SIZE)  # fontsize of the tick labels
    ...
    # plt.savefig(r"C:\Users\asus\Desktop\pics\\" + str(i_point) + '.png')
    """
    2020.12.19 避免画图内存泄露
    """
