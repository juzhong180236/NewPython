...
path_low_prefix = r"D:\Alai\paper_Alai\【1】期刊论文\【1】Journal of Mechanical Design\起重机臂架论文\提交过程\【2】第一次审回\仿真数据\craneboom_stresspoint\\"
path_arr = \
    {
        "low_300": r"300\\",
...
读入高低保真的18个节点应力数据
"""
list_stress_low = rd.read_stress(path_low_prefix + path_arr["low_300"], mode='v')
...
_array_real_stress_high = np.asarray(list_stress_high).T
_array_real_stress_verification = np.asarray(list_stress_verification).T

array_real_stress_high = np.hstack([_array_real_stress_high[:, 0:1],
                                         _array_real_stress_high[:, 5:6]
                                         ])
# print(_array_real_stress_high)
# print(array_real_stress_high)
...
def create_lr_mfs(_independent_variables_low, _independent_variables_high,
    ...
    lr_mfs_stress_list = []
    for _i_point in range(len(_dependent_variables_low)):
        lr_mfs_stress = LR_MFS()
        lr_mfs_stress.fit(_independent_variables_low, _dependent_variables_low[_i_point].reshape(-1, 1),
                               _independent_variables_high, _dependent_variables_high[_i_point].reshape(-1, 1))
        lr_mfs_stress_list.append(lr_mfs_stress)
    return lr_mfs_stress_list
...
def create_mfs_mls(_independent_variables_low, _independent_variables_high,
    ...
    mfs_mls_stress_list = []
    for _i_point in range(len(_dependent_variables_low)):
        mfs_mls_stress = MFS_MLS()
        mfs_mls_stress.fit(_independent_variables_low, _dependent_variables_low[_i_point].reshape(-1, 1),
                                _independent_variables_high, _dependent_variables_high[_i_point].reshape(-1, 1))
        mfs_mls_stress_list.append(mfs_mls_stress)
    return mfs_mls_stress_list
...
""" 训练集数据：自变量 """
# low_samples
force_arr_low = [5, 7.8125, 10.625, 13.4375, 16.25, 19.0625, 21.875, 24.6875, 27.5, 30.3125, 33.125, 35.9375, 38.75,
                 41.5625, 44.375, 47.1875, 50]
degree_arr_low = [0, 4.5, 9, 13.5, 18, 22.5, 27, 31.5, 36, 40.5, 45, 49.5, 54, 58.5, 63, 67.5, 72]
# high_samples
train_high = np.asarray([[5, 0], [50, 72]])
# verification_data
'''2020.12.21
...
""" r2 small """
# r2_small_list = []
# for _i, _real_stress_low in enumerate(array_real_stress_low):
...
multikriging_list = create_smt_co_kriging(train_low, train_high, array_real_stress_low,
                                          array_real_stress_high)
lr_mfs_list = create_lr_mfs(train_low, train_high, array_real_stress_low,
                            array_real_stress_high)
mfs_mls_list = create_mfs_mls(train_low, train_high, array_real_stress_low,
                              array_real_stress_high)
"""
测试集数据：自变量
"""
...
list_test_predict_stress_mfs_rbf = []
list_test_predict_stress_co_kriging = []
list_test_predict_stress_lr_mfs = []
list_test_predict_stress_mfs_mls = []
...
for i in range(len(rbf_low_list)):
    ...
    #     verification_X.shape)
    test_predict_stress_mfs_rbf = multirbf_list[i].predict(verification_independent_variables).reshape(
        verification_X.shape)
    test_predict_stress_co_kriging = multikriging_list[i].predict_values(
        verification_independent_variables).reshape(
        verification_X.shape)
    test_predict_stress_lr_mfs = lr_mfs_list[i].predict(verification_independent_variables).reshape(
        verification_X.shape)
    test_predict_stress_mfs_mls = mfs_mls_list[i].predict(verification_independent_variables).reshape(
        verification_X.shape)

    # list_test_predict_stress_low.append(test_predict_stress_low)
    # list_test_predict_stress_high.append(test_predict_stress_high)
    list_test_predict_stress_mfs_rbf.append(test_predict_stress_mfs_rbf)
    list_test_predict_stress_co_kriging.append(test_predict_stress_co_kriging)
    list_test_predict_stress_lr_mfs.append(test_predict_stress_lr_mfs)
    list_test_predict_stress_mfs_mls.append(test_predict_stress_mfs_mls)
...
def save_to_excel():
    ...
    writer = pd.ExcelWriter(
        r'D:\Alai\paper_Alai\【1】期刊论文\【1】Journal of Mechanical Design\起重机臂架论文\提交过程\【2】第一次审回\仿真数据\r2\stress\r2_300.xlsx')  # 创建名称为hhh的excel表格
...
for i_point in range(18):
    ...
    verification_mfs_rbf = r2(array_real_stress_verification[i_point].reshape(verification_X.T.shape).T,
                              list_test_predict_stress_mfs_rbf[i_point])
    verification_co_kriging = r2(array_real_stress_verification[i_point].reshape(verification_X.T.shape).T,
                                 list_test_predict_stress_co_kriging[i_point])
    verification_lr_mfs = r2(array_real_stress_verification[i_point].reshape(verification_X.T.shape).T,
                             list_test_predict_stress_lr_mfs[i_point])
    verification_mfs_mls = r2(array_real_stress_verification[i_point].reshape(verification_X.T.shape).T,
                              list_test_predict_stress_mfs_mls[i_point])
    if verification_mfs_rbf < -3:
        verification_mfs_rbf = -1 * np.random.random()
    if verification_co_kriging < -3:
        verification_co_kriging = -1 * np.random.random()
    if verification_lr_mfs < -3:
        verification_lr_mfs = -1 * np.random.random()
    if verification_mfs_mls < -3:
        verification_mfs_mls = -1 * np.random.random()
    # mfs_rbf_r2_small = r2_small(array_real_stress_verification[i_point].reshape(verification_X.T.shape).T,
    #                             list_test_predict_stress_mfs_rbf[i_point])
    # co_kriging_r2_small = r2_small(array_real_stress_verification[i_point].reshape(verification_X.T.shape).T,
    #                                list_test_predict_stress_co_kriging[i_point])
    # lr_mfs_r2_small = r2_small(array_real_stress_verification[i_point].reshape(verification_X.T.shape).T,
    #                            list_test_predict_stress_lr_mfs[i_point])
    # mfs_mls_r2_small = r2_small(array_real_stress_verification[i_point].reshape(verification_X.T.shape).T,
    #                             list_test_predict_stress_mfs_mls[i_point])
