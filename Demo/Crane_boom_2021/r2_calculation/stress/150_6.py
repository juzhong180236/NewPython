...
list_real_stress_high = []
for _d in _array_real_stress_verification:
    d = _d.reshape(19, 37)[9, :]
    d1 = d[18]
    list_real_stress_high.append(d1)

array_real_stress_high = np.hstack(
    [_array_real_stress_high[:, 0:2], _array_real_stress_high[:, 4:6],
     np.array(list_real_stress_high).reshape(-1, 1)])
...
# high_samples
train_high = np.asarray([[5, 0], [5, 72], [50, 0], [50, 72], [27.5, 36]])
# verification_data
'''2020.12.21
...
def save_to_excel():
    ...
    writer = pd.ExcelWriter(
        r'D:\Alai\paper_Alai\【1】期刊论文\【1】Journal of Mechanical Design\起重机臂架论文\提交过程\【2】第一次审回\仿真数据\r2\stress\r2_150_6.xlsx')  # 创建名称为hhh的excel表格
