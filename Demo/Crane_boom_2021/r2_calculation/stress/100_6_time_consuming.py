...
读入高低保真的18个节点应力数据
"""
list_stress_low = rd.read_stress(path_low_prefix + path_arr["low_100"], mode='v')
...
array_real_stress_high = _array_real_stress_high
...
for i in range(times):
    ...
    """ 训练集数据：自变量 """
    # low_samples
    force_arr_low = [5, 10, 15, 20, 25, 30, 35, 40, 45, 50]
    degree_arr_low = [0, 8, 16, 24, 32, 40, 48, 56, 64, 72]
    # high_samples
    ...
    #                               array_real_stress_high)
    end_time = time()
    list_training_time.append(np.round((end_time - start_time), 3))
    """
    测试集数据：自变量
    """
    ...
    for i in range(len(multirbf_list)):
        ...
        # test_predict_stress_lr_mfs_listhigh = rbf_high_list[i].predict(verification_independent_variables).reshape(
        #     verification_X.shape)
    ...
    end_time1 = time()
    list_prediction_time.append(np.round((end_time1 - start_time1), 3))

print(list_training_time)
print(list_prediction_time)

print(np.round(np.mean(list_training_time), 3))
print(np.round(np.mean(list_prediction_time), 3))
...
#     writer = pd.ExcelWriter(r'D:\Alai\paper_Alai\【1】期刊论文\【1】Journal of Mechanical Design\起重机臂架论文\提交过程\【2】第一次审回\仿真数据\r2\stress\r2_100_6.xlsx')  # 创建名称为hhh的excel表格
