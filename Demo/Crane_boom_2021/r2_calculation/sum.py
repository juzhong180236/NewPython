...
def draw(_array_results, picture_name):
    ...
    plot_co_kriging1 = plt.plot(x, _array_results[1], label="Co-Kriging", linewidth=1.5,
                                marker='^',
                                markersize=10,
                                color='g',
                                clip_on=False)
    plot_lr_mfs1 = plt.plot(x, _array_results[2], label="LR-MFS", linewidth=1.5,
                            marker='D',
                            markersize=10,
                            color='#ff7f0e',
                            clip_on=False)
    plot_mfs_mls1 = plt.plot(x, _array_results[3], label="MFS-MLS", linewidth=1.5,
                             marker='o',
                             markersize=10,
                             color='#1f77b4',
                             clip_on=False)
    plot_mfs_rbf1 = plt.plot(x, _array_results[0], label="MFS-RBF", linewidth=1.5,
                             marker='s',
                             markersize=10,
                             color='r',
                             clip_on=False)
    ...
    for i in range(8):
        ...
        # ax.text(i, 1.15, _average)
        list_std.append(
            np.std([_array_results[0][i], _array_results[1][i], _array_results[2][i], _array_results[3][i]]))
        list_average.append(_average)
    print(list_average)
    print(np.round(list_std, 3))
    ...
    _plot = plot_mfs_rbf1 + plot_co_kriging1 + plot_lr_mfs1 + plot_mfs_mls1
    labs = [_p.get_label() for _p in _plot]
    # bbox_to_anchor=(x, y, 长, 宽),总图是1*1的 loc是bbox中落在xy位置的角点或中点
    legend_properties = {'weight': 'semibold'}
    plt.legend(_plot, labs, bbox_to_anchor=(0, 0.1, 1, 0.2), borderaxespad=0, ncol=4, loc="lower center", fontsize=18)
    bounds = [[0, 7], [0, 1]]
    ...
    plt.xticks(x_ticks, weight='semibold')
    # bottom, top = plt.ylim()
    # left, right = plt.xlim()
...
draw(array_results_displacement, 'displacement')
draw(array_results_stress, 'stress')