...
读入高低保真的18个节点应力数据
"""
list_deformation_low = rd.read_stress(path_low_prefix + path_arr["low_100"], mode='v')
...
array_real_deformation_high = _array_real_deformation_high
...
def create_smt_co_kriging(_independent_variables_low, _independent_variables_high,
                          _dependent_variables_low, _dependent_variables_high):
    multi_kriging_deformation_list = []
    for _i_point in range(len(_dependent_variables_low)):
        ...
        multi_kriging_deformation_list.append(multi_kriging_deformation)
    return multi_kriging_deformation_list
...
for i in range(times):
    ...
    for i in range(len(mfs_mls_list)):
...
print(np.mean(list_training_time))
print(np.mean(list_prediction_time))
...
#         r'D:\Alai\paper_Alai\【1】期刊论文\【1】Journal of Mechanical Design\起重机臂架论文\提交过程\【2】第一次审回\仿真数据\r2\displacement\r2_100_6.xlsx')  # 创建名称为hhh的excel表格
