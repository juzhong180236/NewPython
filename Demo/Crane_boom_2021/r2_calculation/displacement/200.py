...
读入高低保真的18个节点应力数据
"""
list_deformation_low = rd.read_stress(path_low_prefix + path_arr["low_200"], mode='v')
...
array_real_deformation_high = np.hstack([_array_real_deformation_high[:, 0:2],
                                         _array_real_deformation_high[:, 4:6]
                                         ])
...
def save_to_excel():
    ...
    writer = pd.ExcelWriter(
        r'D:\Alai\paper_Alai\【1】期刊论文\【1】Journal of Mechanical Design\起重机臂架论文\提交过程\【2】第一次审回\仿真数据\r2\displacement\r2_200.xlsx')  # 创建名称为hhh的excel表格
