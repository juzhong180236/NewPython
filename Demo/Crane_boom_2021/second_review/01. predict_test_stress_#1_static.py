...
xlim0, xlim1, ylim0, ylim1 = 30, 32, 4.52, 4.62
...
xlim0, xlim1, ylim0, ylim1 = 52.5, 53, 1.54, 1.7
...
xlim0, xlim1, ylim0, ylim1 = 70, 73, 5.75, 5.82
...
xlim0, xlim1, ylim0, ylim1 = 6, 8, 0.31, 0.34
...
ax.set_title('The predicted stress at test point #1', fontsize=24)
# 角度
len_ = 540
load1 = 20
...
for i in range(10):
    load_temp = test_predict_stress_multi[0][i].flatten() # 索引0是第一个点
    ...
    axins4.plot(list_test_data[0][0:_num],
                ...
                linestyle='-')
multi_temp = []
for _obj in test_predict_stress_multi[0]:
    multi_temp.append(_obj.flatten())
multi_temp1 = []
for _obj in np.array(multi_temp).T:
    multi_temp1.append(_obj.mean())

ax.fill_between(list_test_data[0][0:_num], np.array(multi_temp1) + 0.01147, np.array(multi_temp1) - 0.01147, facecolor="black", alpha=0.5)
axins2.fill_between(list_test_data[0][0:_num], np.array(multi_temp1) + 0.01147, np.array(multi_temp1) - 0.01147, facecolor="black", alpha=0.5)
axins.fill_between(list_test_data[0][0:_num], np.array(multi_temp1) + 0.01147, np.array(multi_temp1) - 0.01147, facecolor="black", alpha=0.5)
axins3.fill_between(list_test_data[0][0:_num], np.array(multi_temp1) + 0.01147, np.array(multi_temp1) - 0.01147, facecolor="black", alpha=0.5)
axins4.fill_between(list_test_data[0][0:_num], np.array(multi_temp1) + 0.01147, np.array(multi_temp1) - 0.01147, facecolor="black", alpha=0.5)
...
# plt.savefig(load_np_txt_path+r"test\stress_#1_20kg.png")
plt.show()
