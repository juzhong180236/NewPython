...
print(sensor_temp)
sigma_stress = _stress_rbf.predict(np.array(sensor_temp)) * 2
...
axins = ax.inset_axes((0.05, 0.33, 0.1, 0.2))
xlim0, xlim1, ylim0, ylim1 = 6, 10, 4.6, 4.9
...
axins2 = ax.inset_axes((0.35, 0.4, 0.08, 0.3))
xlim0, xlim1, ylim0, ylim1 = 66, 67.5, 1.3, 1.5
...
axins3 = ax.inset_axes((0.55, 0.72, 0.2, 0.15))
xlim0, xlim1, ylim0, ylim1 = 134, 139, 3.68, 3.85
...
axins4 = ax.inset_axes((0.86, 0.1, 0.1, 0.1))
xlim0, xlim1, ylim0, ylim1 = 158, 159, 0.32, 0.38
...
for i in range(10):
    load_temp = test_predict_stress_multi[i].flatten()  # 索引0是第一个点
...
ax.fill_between(list_test_data[0][0:_num], np.array(multi_temp1) + sigma_stress, np.array(multi_temp1) - sigma_stress,
                facecolor="black", alpha=0.5)
axins2.fill_between(list_test_data[0][0:_num], np.array(multi_temp1) + sigma_stress,
                    np.array(multi_temp1) - sigma_stress,
                    facecolor="black", alpha=0.5)
axins.fill_between(list_test_data[0][0:_num], np.array(multi_temp1) + sigma_stress,
                   np.array(multi_temp1) - sigma_stress,
                   facecolor="black", alpha=0.5)
axins3.fill_between(list_test_data[0][0:_num], np.array(multi_temp1) + sigma_stress,
                    np.array(multi_temp1) - sigma_stress,
                    facecolor="black", alpha=0.5)
axins4.fill_between(list_test_data[0][0:_num], np.array(multi_temp1) + sigma_stress,
                    np.array(multi_temp1) - sigma_stress,
                    facecolor="black", alpha=0.5)
...
# plt.savefig(load_np_txt_path+r"dynamic_test\stress_#1_15kg.png")
plt.show()
