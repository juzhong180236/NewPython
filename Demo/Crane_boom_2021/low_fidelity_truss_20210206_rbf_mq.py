...
# 训练自变量
forceArr = [5, 10, 15, 20, 25, 30, 35, 40, 45, 50]  # 50 100 150 200 250 300 350 400 450 500
...
def Surrogate_Model():
    ...
    for i in range(cycle_index):
        ...
        deformation_real = list_disp_sum[i]
        rbf_stress = RBF(rbf_type)
        rbf_deformation = RBF(rbf_type)
        w_stress = rbf_stress.fit(fd, stress_real)
        w_disp = rbf_deformation.fit(fd, deformation_real)
        if i == 0:
            stds = rbf_stress.std
        list_w_stress.append(w_stress)
        list_w_dSum.append(w_disp)
        print("\r" + rbf_stress.__class__.__name__ + "程序当前已完成：" + str(round(i / cycle_index * 10000) / 100) + '%',
              end="")
    ...
    with open("C:/Users/laisir/Desktop/" + path_switch[4:-2] + "_rbf.json", "w") as f:
