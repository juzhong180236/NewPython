    ...
    if not os.path.isdir(file):  # 判断是否是文件夹，不是文件夹才打开
        ...
        for line in infile:
            if len(line) == 62 and filename != 'coord':
                ...
                arr_sort.append(str(float(line[22:35].strip()) + float(arr_coord[iCount])))
                arr_sort.append(str(float(line[35:48].strip()) + float(arr_coord[iCount])))
