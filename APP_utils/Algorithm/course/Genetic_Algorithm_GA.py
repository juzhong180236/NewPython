...
class GA(object):
    ...
    def get_fitness_value(self, func, chromosomes_decoded):
        ...
        for i in range(population):
            fitness_values[i, 0] = func(self.obj_func, chromosomes_decoded[i, :])
        # print(fitness_values)
        # 轮盘赌选择法，
        # 计算每个染色体被选择的概率
        divide_num = np.sum(fitness_values)
        if divide_num == 0:  # cg
            divide_num = 1
        probability = fitness_values / divide_num
        # print(probability)
        # print(probability)
        # 得到前n个染色体被选中的概率
    ...
    def fitnessFunction(self, obj_func, para_list):
        """
           定义适应度函数
               返回：
               return: 适应度函数的值
        ...
        # return np.sum(para_list ** 2)
        return obj_func(para_list)
    ...
    def genetic_algorithm(self):
        ...
        for iteration in range(self.max_iter):
            ...
            # 搜索每次迭代的最优解（当前是取最小值，所以是min），以及最优解对应的目标函数（十进制染色体）的取值
            # optimal_values.append(np.max(list(fitness_values)))
            optimal_values.append(np.min(list(fitness_values)))
            index = np.where(fitness_values == min(list(fitness_values)))
            optimal_solutions.append(final_decoded[index[0][0], :])
        # 从每次迭代的最优解集合中找到总体的最优解
        # optimal_value = np.max(optimal_values)
        optimal_value = np.min(optimal_values)
        # 根据全局最优解得到最优解的索引位置
...
# 下面是6个函数，其中para_list是搜索空间，也就是自变量
def Sphere(para_list):
    return np.sum(para_list ** 2)


def Sum_Squares(para_list):
    sum_squares = 0
    for i in range(1, len(para_list) + 1):
        sum_squares += i * para_list[i - 1] ** 2
    return sum_squares
...
def Step(para_list):
    ...
    for i in range(len(para_list)):
        step += np.floor(para_list[i] + 0.5) ** 2
    return step


def Quartic(para_list):
    quartic = 0
    for i in range(1, len(para_list) + 1):
        quartic += i * para_list[i - 1] ** 4
    return quartic + np.random.rand()


def Easom(para_list):
    return -np.cos(para_list[0]) * np.cos(para_list[1]) * \
           np.exp(-(para_list[0] - np.pi) ** 2 - (para_list[1] - np.pi) ** 2)


def Matyas(para_list):
    return 0.26 * (para_list[0] ** 2 + para_list[1] ** 2) \
           - 0.48 * para_list[0] * para_list[1]


# 对这6个函数使用GA测试的入口，分别可以测试空间dv
def test_func(dv, obj_func, mp, cp, max_iter, pop_size):
    """
    ...
    ga = GA(para_array=dv, obj_func=obj_func, mp=mp, cp=cp, max_iter=max_iter, population_size=pop_size)
    solution, value = ga.genetic_algorithm()
    # print('最优解:', solution)
    # print('{0}的最优目标函数值:{1}'.format(obj_func.__name__, value))
...
# 搜索空间，后边的10 2 表示维度
decision_variables_100 = [[-100, 100]] * 12
decision_variables_10 = [[-10, 10]] * 10
decision_variables_128 = [[-1.28, 1.28]] * 10
decision_variables_easom = [[-100, 100]] * 2
decision_variables_matyas = [[-10, 10]] * 2


# 对每一个函数进行具体测试，可以改变的是以下四个参数变量
def test_all_func(mp, cp, max_iter, pop_size):
    """
    :param mp: 突变概率
    :param cp: 交叉概率
    :param max_iter: 最大迭代次数
    ...
    """
    # Sphere
    value_sphere = test_func(decision_variables_100, Sphere, mp, cp, max_iter, pop_size)

    # Sum_Squares
    value_sum_squares = test_func(decision_variables_10, Sum_Squares, mp, cp, max_iter, pop_size)

    # Step
    value_step = test_func(decision_variables_100, Step, mp, cp, max_iter, pop_size)

    # Quartic
    value_quartic = test_func(decision_variables_128, Quartic, mp, cp, max_iter, pop_size)

    # Matyas
    value_matyas = test_func(decision_variables_matyas, Matyas, mp, cp, max_iter, pop_size)

    # Easom
    value_easom = test_func(decision_variables_easom, Easom, mp, cp, max_iter, pop_size)

    value_array = np.array([value_sphere, value_sum_squares, value_step,
                            value_quartic, value_matyas, value_easom])
    # value_array = np.array([value_sum_squares, value_quartic, value_matyas, value_easom])
    return value_array
...
def draw_results(para_array, which_para):
    ...
    for i_para, i_process in zip(para_array, i_array):
        if which_para == 'Mutation Probability':
            result = test_all_func(i_para, 0.8, 250, 60)
        elif which_para == 'Crossover Probability':
            result = test_all_func(0.1, i_para, 250, 60)
        elif which_para == 'Max Iteration':
            result = test_all_func(0.1, 0.8, i_para, 60)
        elif which_para == 'Population Size':
            result = test_all_func(0.1, 0.8, 250, i_para)
        else:
            return
...
# 改变参数——突变概率
mp_array = np.arange(0.0001, 0.5, 0.05)
draw_results(mp_array, "Mutation Probability")

# 改变参数——交叉概率
cp_array = np.arange(0.4, 0.99, 0.05)
# draw_results(cp_array, "Crossover Probability")

# # 改变参数——最大迭代次数
max_iter_array = np.arange(10, 501, 50)
# draw_results(max_iter_array, "Max Iteration")

# # 改变参数——种群数量
pop_size_array = np.arange(20, 101, 10)
