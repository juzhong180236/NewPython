...
def realXYZ():
    ...
    for i in range(5):
        ...
        yy = Duplicated_list(list_y, 'coords', i)
        ss = Duplicated_list(list_stress, 'stressOrdSum', i)
        dd = Duplicated_list(list_dSum, 'stressOrdSum', i)
        plt.plot(d_pred, stress_pred, color='#0000ff', marker='+', linestyle='-.',
                 label=('' if i == 0 else '_') + 'z-predict')
        plt.plot(d, ss, color='#ff00ff', marker='+', linestyle='-',
