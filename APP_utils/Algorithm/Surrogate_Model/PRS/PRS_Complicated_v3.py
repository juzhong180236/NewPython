...
def readExcel(path, sheet, row_num, column_num):
    ...
    data_X = np.empty((row_num, column_num))  # 用于存储数据的array，相较于np.ones只分配不初始化
...
class PRS(object):
    ...
    def fit(self, X, Y):
        ...
        for i in range(X.shape[0]):  # 遍历输入值X的每个元素
            ...
            list_PRS_result.append(arr_combine)  # 所有元素的Gramian矩阵
        PRS_result = np.array(list_PRS_result)  # 将list转为ndarray
