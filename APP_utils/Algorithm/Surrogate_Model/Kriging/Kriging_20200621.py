...
class Kriging(object):
    def __init__(self, parameters=np.array([0.5, 2])):
        self.X = None
        self.Y = None
    ...
    def fitness(self, parameters):
        ...
        return np.log(np.abs(self.sigma2)) * (self.F.shape[-1] / 2) + np.log(R)
    ...
    def fit(self, X, Y):
        ...
        res = minimize(self.fitness, self.parameters,
                       bounds=np.array([1e-4, 1e4] * self.parameters.shape[0]).reshape(-1, 2),
                       method='L-BFGS-B')
        self.parameters[0], self.parameters[1] = res.x[0], res.x[1]
...
if __name__ == "__main__":
    ...
    ye_pred, xa = gpr.predict(x_pred.reshape(-1, 1))
    ...
    XC_point = np.array([0.1, 0.2, 0.3, 0.8, 0.9])
