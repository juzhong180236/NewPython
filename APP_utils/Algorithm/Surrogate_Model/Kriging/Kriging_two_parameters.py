...
class Kriging(object):
    ...
    def gaussian_kernel(self, x1, x2):
        ...
        if x1.shape[-1] != 1:
            for x1_ele in x1:
                list_result.append(np.exp(np.sum(-self.theta * (x1_ele - x2) ** self.p, axis=-1)).ravel())
        else:
            for x1_ele in x1:
                list_result.append(np.exp(-self.theta * (x1_ele - x2) ** self.p).ravel())
        return np.asarray(list_result)

    def negative_log_likelihood_loss(self, parameters):
        self.theta, self.p = parameters[0], parameters[1]
    ...
    def train(self, x, y):
        ...
        # 拟牛顿法：BFGS
        # 未知方法
        print(self.theta, self.p)

        res = minimize(self.negative_log_likelihood_loss, np.asarray([self.theta, self.p]),
                       bounds=[(0.1, 10), (0.5, 2)],
                       method='L-BFGS-B')
        print(self.theta, self.p)
        ...
        self.theta, self.p = res.x[0], res.x[1]
