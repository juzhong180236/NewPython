...
class coKriging(object):
    ...
    def fit(self, Xc, Yc, Xe, Ye):
        ...
        self.C_parameters = self.C_krig.parameters[2]
        ...
        self.d = Ye - self.C_parameters * self.Yc_Xe

        # D
        self.D_krig = Kriging(parameters=self.Dpara_arr)
        self.D_krig.fit(self.Xe, Ye)
        # self.C_parameters = self.D_krig.fit()[2]
        ...
        C2 = self.C_parameters * self.C_sigma2 * C_matrix_ce
        C3 = self.C_parameters * self.C_sigma2 * C_matrix_ce.T
        C4 = self.C_parameters ** 2 * self.C_sigma2 * C_matrix_ee + self.D_sigma2 * D_matrix_ee
    ...
    def predict(self, X_pre):
        ...
        C_matrix_cx = self.corelation(self.C_krig.gaussian, self.Xc, X_pre,
                                      self.C_krig.parameters)  # C的cx矩阵
        D_matrix_ex = self.corelation(self.D_krig.gaussian, self.Xe, X_pre,
                                      self.D_krig.parameters)  # D的ex矩阵
        c1 = self.C_parameters * self.C_sigma2 * C_matrix_cx
        c2 = (self.C_parameters ** 2 * self.C_sigma2 + self.D_sigma2) * D_matrix_ex
        c = np.vstack((c1, c2))
        Y_pre = self.beta + c.T.dot(self.C_inverse).dot(self.Y - self.co_F.dot(self.beta))
        return Y_pre

    def model(self):
        model1 = [self.beta, self.C_sigma2, self.C_parameters,
                  self.C_krig.parameters, self.D_krig.parameters]
        return model1


if __name__ == "__main__":
    parameter_array_c = np.array([0.5, 2, 8])
    parameter_array_d = np.array([0.5, 2])
    ...
    kriging = Kriging(parameters=parameter_array_d)
