...
class Sur_MFSMLS(object):
    ...
    **kwargs: 输入权重函数的类型，e.g., weight_type='cubic_spline', 总共三个权重函数可供选择
    ...
    def __init__(self):
        self.XL = None  # 最好在这里加一条np.array因为如果传入的数据是np.mat的话，那么后面的矩阵运算可能有错误，比如想元素相乘，结果实现的是矩阵想乘
        self.YL = None
        self.XH = None
        self.YH = None
        self.Xtest = None
        self.ntest = None  # 测试点样本个数
        self.nH = None  # 高保真训练样本点
        self.ndv = None  # 问题维度/高保真维度/变量个数
        self.weight_type = None
        self.degree = None  # 默认基函数的次数为2
        self.P = None
        pass

    def fit(self, XL, YL, XH, YH, **kwargs):
...
if __name__ == "__main__":
    ...
    example1 = Sur_MFSMLS()
    example1.fit(XL, YL, XH, YH, weight_type='gaussian')
    yhat = example1.predict(Xtest)
    # print(yhat)
