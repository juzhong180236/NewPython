...
arr_coord = []
coordfile = open(path + "NLIST.lis", 'rt')
file_coord_result = ''
for everyline in coordfile:
    if len(everyline) == 76 and everyline[0:9].strip() in set_surele:
...
print(len(arr_coord)/3)
file_coord_result = ','.join(arr_coord) + '\n'  # 带初始坐标信息

text_create('coord', file_coord_result.rstrip('\n'))
